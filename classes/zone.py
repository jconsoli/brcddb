...
Defines the zone classes ZoneCfgObj, ZoneObj, and AliasObj
...
+-----------+---------------+---------------------------------------------------------------------------------------+
| 4.0.3     | 20 Oct 2024   | Added default value to r_get() and r_alert_obj()                                      |
+-----------+---------------+---------------------------------------------------------------------------------------+
...
class ZoneCfgObj:
    ...
    def s_copy(self, zonecfg, resolve=False):
        """Copy self to a new zone configuration

        :param zonecfg: Name of new zone configuration to create
        :type zonecfg: str
        :param resolve: If True, resolve all aliases. This is useful for creating a zone cfg to match the effective
        :type resolve: bool
        :return: Zone configuration object. None if zone configuration already exists of could not find the fabric obj
        :rtype: brcddb.classes.alert.ZoneCfgObj, None
        """
        fab_obj = self.r_fabric_obj()
        if fab_obj is None or fab_obj.r_zonecfg_obj(zonecfg) is not None:
            return None
        if resolve:
            mem_l = list()
            for zone in self.r_members():
                zone_obj = fab_obj.r_zone_obj(zone)
                if zone_obj is not None:  # It might be none if building the zone DB is a work in progress
                    for alias in zone_obj.r_members() + zone_obj.r_pmembers():
                        alias_obj = fab_obj.r_alias_obj(mem)
                        if alias_obj is not None:
                            mem_l.extend(alias_obj.r_members())
