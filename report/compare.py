...
def _bool_compare(r_obj, ref, b_flag, c_flag, control_tbl):
    ...
    if skip_flag:
        return 0
    if c_flag is None:
        brcddb_util.add_to_obj(r_obj, new_ref, dict(b=str(b_flag), c='', r=_REMOVED))
        return 1
    if c_flag != b_flag:
        brcddb_util.add_to_obj(r_obj, new_ref, dict(b=str(b_flag), c=str(c_flag), r=_CHANGED))
        return 1
...
def _unknown_compare(r_obj, ref, b_obj, c_obj, control_tbl):
    ...
    brcddb_util.add_to_obj(r_obj, new_ref, dict(b=buf, c='', r=_INVALID_REF))
