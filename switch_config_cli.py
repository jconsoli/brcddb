...
Version Control::
    ...
    +===========+===============+===================================================================================+
    | 1.0.0     | 11 Feb 2023   | Initial launch                                                                    |
    +-----------+---------------+-----------------------------------------------------------------------------------+
"""
...
_DEBUG_i = 'test/test_61'
_DEBUG_o = 'test/Dir61_config'
_DEBUG_sup = False
_DEBUG_log = '_logs'
_DEBUG_nl = False
...
_preamble_l = [
    ...
    '#     continuing',
    '#   * Insistent Domain ID is automatically set for ficon switches. Therefore, setting it is not an',
    '#     option in the configure commands.',
    '',
]
_chassis_config_l = [
    '########################################################',
...
_zoning_l = [
    ...
    '# in multiple places so make sure all are replaced.',
    '',
]
_enable_l = [
    '',
...
def _switch_commands(switch_d):
    ...
    for port in port_l:
        temp_l = port.split('/')
        slot_port_l = slot_d.get(temp_l[0])
        if slot_port_l is None:
            slot_port_l = list()
            slot_d.update({temp_l[0]: slot_port_l})
        slot_port_l.append(int(temp_l[1]))
    ...
    if len(xisl) > 0:
        cmd_l.extend(['# WARNING: The "-lisldisable" was added in FOS v9.1. If the command below',
                      '# fails, retry without the "-lisldisable" and disable XISL in the next step.',
                      ''])
    cmd_l.extend(['lscfg --create ' + fid + xisl + ' -' + switch_d['switch_info']['switch_type'] + ' -force',
                  '',
                  'setcontext ' + fid,
                  '',
                  'switchdisable',
    ...
    configure_l = [dict(t='#   * Domain: ', a=str(switch_did)),
                   dict(t='#   * WWN Based persistent PID: ', a='no'),
                   dict(t='#   * Allow XISL Use: ', a='yes' if switch_xisl else 'no'),
                   dict(t='#   * Insistent Domain ID Mode: ', a='yes' if switch_idid else 'no'),
                   dict(t='#   * Disable Default PortName: ',
                        a='no' if switch_port_name == 'default' else 'yes'),
                   dict(t='#   * Dynamic Portname: ', a='on' if switch_port_name == 'dynamic' else 'off'),
                   dict(t='#   * Virtual Channel parameters: ', a='no'),
                   dict(t='# F-Port login parameters: ', a='no' if switch_dup_wwn == 0 else 'yes')]
    if switch_dup_wwn != 0:
        configure_l.append(dict(t='#   * Enforce FLOGI/FDISC login: ', a=str(switch_dup_wwn)))
    for d in configure_l:
        cmd_l.append(d['t'] + d['a'])
    ...
    for slot, port_l in slot_d.items():
        for port_range in gen_util.int_list_to_range(port_l):
...
def _port_commands(switch_d):
    ...
    # Reserve ports
    for slot, port_l in switch_d['_slot_d'].items():
        if slot != '0':
            break
        for port_range in gen_util.int_list_to_range(port_l):
    ...
    for buf in ['portname ' + pd[k]['port'] + ' -n "' + pd[k]['port_name'] + '"' for k in
                brcdapi_port.sort_ports(pd.keys()) if isinstance(pd[k]['port_name'], str)]:
        if i >= _MAX_LINES:
            cmd_l.append('')
            i = 0
        else:
...
def _enable_commands(switch_d):
    ...
    if switch_d['switch_info']['enable_ports']:
        index_l = [d['index'] for d in switch_d['port_d'].values()]
        if len(index_l) > 0:
            cmd_l.append('portenable -i ' + str(min(index_l)) + '-' + str(max(index_l)) + ' -f')
